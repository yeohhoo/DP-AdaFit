...
class TrainLoop:
        ...
        for epoch in range(total_epoch):
            with BatchMemoryManager(
                ...
                for batch, cond in memory_safe_data_loader:
                    ...
                    self.run_step(batch, cond, privacy_engine, epoch)
    ...
    def run_step(self, batch, cond, privacy_engine, epoch):
    ...
    def forward_backward(self, batch, cond, epoch):
        ...
        if self.args.global_rank == 0:
            log_loss_dict(
                self.diffusion, t, {k: v * weights for k, v in losses.items()}, epoch, self.writer
            )
# self.writer.add_scalar("grad_cosine_similarity/train", grad_cosine_similarity, self.step)
        dist.barrier()
...
def log_loss_dict(diffusion, ts, losses, epoch, writer):
