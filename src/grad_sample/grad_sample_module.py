...
# from ..optimizers.utils import param
...
class GradSampleModuleAugmented(AbstractGradSampleModule):
    """
    Hooks-based implementation of AbstractGradSampleModule

    Computes per-sample gradients with augmentation multiplier using custom-written methods for each layer.
    ...
    def __init__(
        ...
        *,
        ...
        self.dp_paras = ['lora_E','weight','bias' ]
    ...
    def capture_backprops_hook(
        ...
        grad_samples = grad_sampler_fn(module, activations, backprops, self.K)
        ...
        # (See #417 for details)
        for n, p in trainable_parameters(module):
            for dp_name in self.dp_paras:
                if dp_name in n:
                    p._forward_counter -= 1
...
def _get_batch_size(*, module: nn.Module, batch_dim: int, K: int) -> int:
    """
