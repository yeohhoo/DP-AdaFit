...
from .grad_sample_module import GradSampleModuleAugmented
...
def register_grad_sampler(
    ...
    def decorator(f):
        ...
        for target_class in target_classes:
            GradSampleModuleAugmented.GRAD_SAMPLERS[target_class] = f
        return f

    return decorator


def wrap_model(model: nn.Module, grad_sample_mode: str, K:int=0, *args, **kwargs):
    cls = get_gsm_class(grad_sample_mode)
    if grad_sample_mode == "functorch":
        kwargs["force_functorch"] = True
    return cls(model, K ,*args, **kwargs)
...
def get_gsm_class(grad_sample_mode: str) -> Type[AbstractGradSampleModule]:
    ...
    if grad_sample_mode in ["hooks", "functorch"]:
        return GradSampleModuleAugmented
