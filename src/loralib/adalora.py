...
class SVDLinear(nn.Linear, LoRALayer):
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, 'lora_A'):
            ...
            # nn.init.normal_(self.lora_E, mean=0.0, std=0.02)  # for test
...
class SVDEmbedding(nn.Embedding, LoRALayer):
    # SVD-based adaptation implemented in a dense layer
    def __init__(
        self,
        num_embeddings: int,
    ...
    def train(self, mode: bool = True):
        ...
        if self.merge_weights and self.merged:
            # Make sure that the weights are not merged
            if self.r > 0:
                self.weight.data -= (self.lora_B @ (self.lora_A*self.lora_E)
    ...
    def forward(self, x: torch.Tensor):
        if self.r > 0 and not self.merged:
            ...
            if self.r > 0:
                after_A = F.embedding(
                    x, (self.lora_A*self.lora_E).T, self.padding_idx, self.max_norm,
                    self.norm_type, self.scale_grad_by_freq, self.sparse
                )
                result += (after_A @ self.lora_B.T
                           ) * self.scaling / (self.ranknum+1e-5)
            return result
        else:
...
class SVDConv2d(nn.Conv2d, LoRALayer):
    # SVD-based adaptation implemented in a dense layer
    def __init__(
        self, 
        in_channels: int, 
        ...
        **kwargs
        ...
        if r > 0:
            ...
            # )
            # self.lora_E = nn.Parameter(
            #     self.weight.new_zeros(r*kernel_size, 1)
            # )
            # self.lora_B = nn.Parameter(
            #     self.weight.new_zeros((out_channels*kernel_size, r*kernel_size))
            # )
            self.lora_A = nn.Parameter(
                self.weight.new_zeros((r, in_channels*kernel_size*kernel_size))
            )
            self.lora_E = nn.Parameter(
                self.weight.new_zeros(r, 1)
            )
    ...
    def forward(self, x: torch.Tensor):
        if self.r > 0 and not self.merged:
            return F.conv2d(
                x, 
                self.weight + (self.lora_B @ (self.lora_A*self.lora_E)
                                 ).view(self.weight.shape
                                        ) * self.scaling / (self.ranknum+1e-5),
...
class SVDConv1d(nn.Conv1d, LoRALayer):
    # SVD-based adaptation implemented in a dense layer
    def __init__(
        self, 
        in_channels: int, 
        ...
        with torch.no_grad():
            ...
            for n,p in model.named_parameters():
                if "lora_E" in n: 
                    ...
                    if self.tb_writter is not None and self.global_step%self.log_interval==0 and self.global_rank == 0:
