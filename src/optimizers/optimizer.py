...
# from optimizers.utils import params
...
class DPOptimizer(Optimizer):
    ...
    def __init__(
        ...
        *,
        ...
        self.params, self.dp_params = self.params_func
    ...
    @property
    def params_func(self) -> List[nn.Parameter]:
        """
        Returns a flat list of ``nn.Parameter`` managed by the optimizer
        """
        return params(self, module=self.module)
    ...
    def clip_and_accumulate(self):
        ...
        for p in self.params:
            if id(p) in self.dp_params:
                ...
                grad_sample = self._get_flat_grad_sample(p)
            if id(p) in self.dp_params:
                # print(f"{id(p)}need clip!!!!!!!")
                grad = contract("i,i...", per_sample_clip_factor, grad_sample)
                # print(f"per_sample_clip_factor={per_sample_clip_factor.shape}")
                # print(f"grad_sample={grad_sample.shape}")
    ...
    def add_noise(self):
        ...
        for p in self.params:
            ...
            if id(p) in self.dp_params:
                # print(f"{id(p)}need noise!!!!!!!")
                p.grad = (p.summed_grad + noise).view_as(p)
            else:
                p.grad = (p.summed_grad).view_as(p)
                # print(f"{id(p)}dont need noise?????")
