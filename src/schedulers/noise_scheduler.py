...
class DynamicExponentialNoise(_NoiseScheduler):
    """
    Multiplies the noise_multiplier by gamma every epoch (so the gamma factors accumulate).
    This means that:
        - For 0 < gamma <= 1
    ...
    def __init__(self, optimizer: DPOptimizer, *, gamma: float, is_loss_decrease:int = 0, is_the_early_stage: int = 0, last_epoch: int = -1):
        """
        ...
        Args:
            ...
            gamma: Multiplicative factor of learning rate decay.
            last_epoch: The index of last epoch. Default: -1.
            is_loss_decrease:The loss is decrease. Default:0
            is_the_early_stage:The early stage of training. Default:0
        """
        self.gamma = gamma
        self.is_loss_decrease = is_loss_decrease
        self.is_the_early_stage = is_the_early_stage

        super().__init__(optimizer, last_epoch=last_epoch)

    def get_noise_multiplier(self):
        # print("dynamic noise scheduler append")
        if self.last_epoch == 0:
          return self.optimizer.noise_multiplier
        elif self.is_loss_decrease == -1 and self.is_the_early_stage == -1:
          return self.optimizer.noise_multiplier * self.gamma #round(1 / self.gamma, 2)
        elif self.is_loss_decrease == 0 and self.is_the_early_stage == 0:
          return self.optimizer.noise_multiplier
        else:
          return self.optimizer.noise_multiplier # * self.gamma
