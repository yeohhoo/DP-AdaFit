...
DATASETS = {
    'imagenet': {
        'train': 'H:\\dataset\\imagenet\\imagenet\\train',
        'valid': 'H:\\dataset\\imagenet\\imagenet\\val',
        'num_classes': 1000,
        'img_size': 256,
        'crop_size': 224,
    ...
    'cifar10': {
        'train': "/hy-tmp/cifar_train",
        'valid': "/hy-tmp/cifar_test",
        'num_classes': 10,
        'img_size': 32,
        'crop_size': 28,
        'max_batch_size': 4096,
...
def populate_dataset(params):
    """
    Configure the parameters of the dataset
    """
    ...
    if params.crop_size is None:
        ...
        # print(f"params.crop_size={params.crop_size}")


NORMALIZE_IMAGENET = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                          std=[0.229, 0.224, 0.225])
NORMALIZE_CIFAR = transforms.Normalize(mean=[0.4914, 0.4822, 0.4465],
                                       std=[0.2023, 0.1994, 0.2010])


def getImagenetTransform(name, img_size=256, crop_size=224, normalization=True,
                         as_list=False, differentiable=False, params=None):
    transform = []
    if differentiable:
        assert False
    else:
            ...
            postprocess.append(
                transforms.Lambda(
                    lambda crops: torch.stack(
                        [NORMALIZE_IMAGENET(crop) for crop in crops]
                    )
                )
            )
        else:
...
def build_transform(is_train, args):
    ...
    t.append(transforms.Normalize(NORMALIZE_IMAGENET.mean,
                                  NORMALIZE_IMAGENET.std))
...
def get_data_loader(params, split, transform, shuffle):
    ...
    if transform == "deit":
        ...
        transform = getImagenetTransform(transform,
    ...
    # Data
    data = DatasetFolder(root=DATASETS[params.dataset][split],
                         transform=transform)
    l_data = len(data)
    if params.proportion < 1:
        indices = np.random.choice(l_data,
                                   int(params.proportion*l_data),
                                   replace=False)
...
def get_data_loader_augmented(params, split, transforms, shuffle):
    ...
    # Data
    data = MultiViewDatasetFolder(root=DATASETS[params.dataset][split],
