...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        self.trainable_names = [ "gamma",] #"bias","norm" ,"output_blocks","time_embed","emb_layers","input_blocks","middle_block",
        ...
        if th.cuda.is_available():
            ...
            # n, n_r = check_model(self.model)
            # print(f"model parameter+parameter_require={n},{n_r}")


            self.use_ddp = True
            self.dpddp_model = DDP(
                self.model
            )
            ...
            if self.resume_step > 0:
                ...
                if self.args.global_rank == 0:
                    logger.log(f"dpddp_model parameter+parameter_require={n},{n_r};{n_num},{n_r_num},{named_p}")
                # n, n_r = check_model(self.dpddp_model)
                # print(f"dpddp_model parameter+parameter_require={n},{n_r}")
    ...
    def run_loop(self):
        ...
        for epoch in range(total_epoch):
            print(f"epoch={epoch}")
            for batch, cond in self.train_loader:
                    ...
                    self.run_step(batch, cond, epoch)
                    ...
                    #         return
                    if ((self.step+1) % self.log_interval == 0) and (self.args.global_rank == 0):
                        # self.log_step()
                        logger.dumpkvs()
    ...
    def run_step(self, batch, cond, epoch):
    ...
    def forward_backward(self, batch, cond, epoch):
        ...
        if self.args.global_rank == 0:
            log_loss_dict(
                self.diffusion, t, {k: v * weights for k, v in losses.items()}, epoch, self.writer, self.opt, self.log_interval, self.step
            )
# self.writer.add_scalar("grad_cosine_similarity/train", grad_cosine_similarity, self.step)
        dist.barrier()
...
def check_model(model):
    ...
    for name, param in model.named_parameters():
        ...
        if param.requires_grad == True:
            ...
            named_p.append((name, param.shape))
...
def log_loss_dict(diffusion, ts, losses, epoch, writer, opt, log_interval, step):
    for key, values in losses.items():
        logger.logkv_mean(key, values.mean().item())
        if (step % log_interval == 0):  #(not opt._is_last_step_skipped) and 
